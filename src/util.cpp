...
const char * const BITCOIN_CONF_FILENAME = "privat.conf";
const char * const BITCOIN_PID_FILENAME = "privatd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "privat" is a composite category enabling all Privat-related debug output
            if(ptrCategory->count(string("privat"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\PrivatCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\PrivatCore
    // Mac: ~/Library/Application Support/PrivatCore
    // Unix: ~/.privatcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "PrivatCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/PrivatCore";
#else
    // Unix
    return pathRet / ".privatcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty privat.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override privat.conf
